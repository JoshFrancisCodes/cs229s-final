...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Use the masks for indexing when needed
        self.kv_cache = self.kv_cache[:, :min(batch_dim, 32), :, :, :][:, :, :, :min(seq_dim, 512), :]
    ...
    def forward(self, x):
        ...
        if self.kv_enabled:
            ...
            self.seq_pos = T
            # update the kv cache
            self.kv_cache = torch.stack((k, v), dim=0) # should be (2, B, nh, T, hs)
            ### END YOUR CODE HERE
            pass
    ...
    def decode(self, x): # note: x now only includes the tokns *after* the KV cache
        ...
        old_k = self.kv_cache[0, :B, :, :self.seq_pos, :]
        old_v = self.kv_cache[1, :B, :, :self.seq_pos, :]
        new_k = torch.cat((old_k, k), dim=2) # (1, B, nh, T, hs)
        new_v = torch.cat((old_v, v), dim=2) # (1, B, nh, T, hs)

        self.kv_cache = torch.stack((new_k, new_v), dim=0)
        calc_k = new_k.squeeze(dim=0)
        calc_v = new_v.squeeze(dim=0)

        # compute manual implmentation of attention using the kv cache
        att = (q @ calc_k.transpose(-2, -1)) * (1.0 / math.sqrt(calc_k.size(-1)))
        
        # print out bias
        # print out att
        att = att.masked_fill(self.bias[:,:,self.seq_pos:self.seq_pos + T,:self.seq_pos + T] == 0, -float('Inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = att @ calc_v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
...
class GPT(nn.Module):
    ...
    def decode(self, idx):
        ...
        tok_emb = self.transformer.wte(idx.to(torch.int)) # token embeddings of shape (b, t, n_embd)
        pos_emb = self.transformer.wpe(pos) # position embeddings of shape (t, n_embd)
    ...
    def generate_kv(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        logits, _ = self.forward(idx) # Shape: (Batch, seq_len, vocab_len)
        ...
        if top_k == 1:
            ...
            idx_next = torch.multinomial(probs, num_samples=1)
        # save the new tokens
        new_tokens = idx_next#torch.cat((new_tokens.to(device='cuda'), idx_next), dim=1)
        ...
        for _ in range(max_new_tokens-1):
            ...
            new_tokens = torch.cat((new_tokens.to(device='cuda'), idx_next), dim=1)
            new_tokens = new_tokens.int()
    ...
    def generate_speculative(self, idx, max_new_tokens, draft_model, temperature=1.0, top_k=None, num_speculative=4):
        ...
        while idx.size(1) < idx_length_original+max_new_tokens:
            ...
            idx=idx_cond,
            max_new_tokens=min(num_speculative, self.config.block_size-idx.size(1)),
            temperature=temperature,
            top_k=1
        )

            # obtain the logits from the main model by passing in the idx_speculative
            all_logits, loss = self(idx_speculative) # idx_spec = (idx+num_speculative, vocab)
            # all_logits = (1, idx+num_speculative, vocab)
            ...
            for i in range(num_speculative, -1, -1):
              # pluck the logits at the current position and scale by desired temperature
              logits = all_logits[:, -1 - i, :] / temperature # (1, vocab) at i

              # optionally crop the logits to only the top k options
              if top_k is not None:
                if top_k == 1:
                    idx = torch.cat((idx, torch.argmax(logits, dim=-1)), dim=1) # using argmax here preserves RNG state
                    continue
                v, _ = torch.topk(logits, min(top_k, logits.size(-1)))
                logits[logits < v[:, [-1]]] = -float('Inf')
